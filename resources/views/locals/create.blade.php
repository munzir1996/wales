...
<div class="w-full p-4 mt-8 bg-white rounded-lg xl:p-6">
    <h1 class="text-lg font-medium text-gray-700 capitalize sm:text-xl md:text-2xl">إنشاء محلية</h1>

    <form action="{{route('locals.store')}}" class="mt-6 space-y-5" method="POST">
        @csrf
        <div class="grid grid-cols-2 gap-8 mt-6">
            <div>
                <label for="local" name="name"
                    class="block text-sm text-gray-700 capitalize dark:text-gray-200">الأسم</label>
                <input type="text" name="name" placeholder="المحلية" value="{{old('name')}}"
