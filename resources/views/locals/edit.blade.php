...
@endpush
@push('breadcrumb')
<a href="{{route('locals.index')}}" class="text-indigo-600 dark:text-gray-200 hover:underline">
    المحلية
</a>
...
<div class="w-full p-4 bg-white rounded-lg xl:p-6">
    <h1 class="text-lg font-medium text-gray-700 capitalize sm:text-xl md:text-2xl">تعديل المحلية</h1>

    <form action="{{route('locals.update', $local->id)}}" class="mt-6 space-y-5" method="POST">
        @csrf
        {{ method_field('PUT') }}
        <div class="grid grid-cols-2 gap-8 mt-6">
            <div>
                <label for="local" name="name"
                    class="block text-sm text-gray-700 capitalize dark:text-gray-200">الأسم</label>
                <input type="text" name="name" placeholder="المحلية" value="{{$local->name}}"
            ...
            <div>
                ...
                <select name="state_id"
                    ...
                    <option value="{{$state->id}}"
                        {{ $state->id == $local->state_id ? 'selected' : ''}}>
                        {{$state->name}}
                    </option>
                    @endforeach
                </select>
