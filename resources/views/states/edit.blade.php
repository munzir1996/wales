...
<div class="w-full p-4 bg-white rounded-lg xl:p-6">
    <h1 class="text-lg font-medium text-gray-700 capitalize sm:text-xl md:text-2xl">تعديل الولاية</h1>

    <form action="{{route('states.update', $state->id)}}" class="mt-6 space-y-5" method="POST">
        @csrf
        {{ method_field('PUT') }}
        <div>
            <label for="state" name="name" class="block text-sm text-gray-700 capitalize dark:text-gray-200">الأسم</label>
            <input type="text" name="name" placeholder="الولاية" value="{{$state->name}}" class="block w-full px-3 py-2 mt-2 text-gray-600 bg-white border @error('name') border-red-500 rounded-md focus:border-red-400 focus:outline-none focus:ring focus:ring-red-300 focus:ring-opacity-40 @enderror" required>
            @error('name')
