...
<div class="w-full p-4 bg-white rounded-lg xl:p-6">
    ...
    <form action="{{route('users.update', $user->id)}}" class="mt-6 space-y-5" method="POST">
        ...
        <div>
            <label for="password" name="password" class="block text-sm text-gray-700 capitalize dark:text-gray-200"> كلمه المرور</label>
            <input type="password" name="password" placeholder=" كلمه المرور" value="{{old('password')}}" class="block w-full px-3 py-2 mt-2 text-gray-600 bg-white border @error('name') border-red-500 rounded-md focus:border-red-400 focus:outline-none focus:ring focus:ring-red-300 focus:ring-opacity-40 @enderror" required>
            @error('password')
                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
