...
@section('body')
    ...
    <div class="w-full p-4 bg-white rounded-lg xl:p-6">
        ...
        <form action="{{ route('users.store') }}" class="mt-6 space-y-5" method="POST">
            @csrf
            <div>
                <label for="state" name="name"
                    class="block text-sm text-gray-700 capitalize dark:text-gray-200">الأسم</label>
                <input type="text" name="name" placeholder="الأسم" value="{{ old('name') }}"
            ...
            <div>
                <label for="email" name="email" class="block text-sm text-gray-700 capitalize dark:text-gray-200">البريد
                    الالكتروني</label>
                <input type="email" name="email" placeholder="البريد الاكتروني" value="{{ old('email') }}"
                    class="block w-full px-3 py-2 mt-2 text-gray-600 bg-white border @error('name') border-red-500 rounded-md focus:border-red-400 focus:outline-none focus:ring focus:ring-red-300 focus:ring-opacity-40 @enderror"
                    required>
                @error('email')
                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
